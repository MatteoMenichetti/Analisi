...
\begin{theorem}[Fermat]\label{th:fermat_an1}
    Sia $f$ definita in $[a,b]$ e sia $\boldsymbol{x_0}\in(a,b)$ un \textbf{punto di massimo (o minimo) relativo}. \textbf{Se} $\boldsymbol f$ \textbf{è derivabile in} $\boldsymbol{x_0}$, allora $\boldsymbol{f'(x_0)=0}$.
\end{theorem}
...
\begin{theorem}[Lagrange]\label{th:lagrange_analisi1}
...
Dato il Teorema di Fermat (Teorema \ref{th:fermat_an1}) è possibile la seguente osservazione.
...
Dal solo Teorema di Fermat (Teorema \ref{th:fermat_an1}) non è possibile capire se $x_0$ è un estremo locale. Per scoprire se un \gls{punto stazionario} è un estremo sono utili i seguenti Teoremi. È supposto che che $f$ sia derivabile almeno due volte in $x_0$.
