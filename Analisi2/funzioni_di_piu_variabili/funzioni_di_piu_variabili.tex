...
\begin{example}[Ricerca massimi e minimi vincolati]\footnote{Scritto 15 Settembre 2025.}
	...
	\end{figure}
	\paragraph{Svolgimento:} Sia $D=D_1\cup D_2$, dove $D_1 = D \cap \{y = \sqrt{2-2x^2}\}$ e $D_2 = D \cap \{x = \sqrt{\frac{2-y^2}{2}}\}$, allora
	\begin{itemize}
		\item [$\boldsymbol{D_1}$:] 
		\begin{equation*}
			f_{|_{D_1}} = f(x, \sqrt{2-2x^2}) = 1+ x^2 - (\sqrt{2-2x^2})^2 = 1+x^2-2+2x^2=3x^2-1,
		\end{equation*}
		quindi dallo studio del segno di $f'_{|_{D_1}}$ ($f'_{|_{D_1}} = 6x\geq0$? Vedere Figura \ref{fig:f'_vincolata>0}) è ottenuto che $(0,\sqrt{2})$ minimo assoluto e $(\pm 1, 1)$ massimi assoluti di $f_{|_{D_1}}$ in $[-1,1]$.
		\begin{figure}[ht]
			\centering
		...
		\end{equation*}
		quindi dallo studio del segno di $f'_{|_{D_2}}$ ($f'_{|_{D_2}} = -3y^2\geq0$? Vedere Figura \ref{fig:f'_vincolata_1>0}) è ottenuto che $(1,0)$ è il massimo assoluto e $(1, \pm\sqrt{2})$ i minimi assoluti di $f_{|_{D_2}}$ in $[-\sqrt{2}, \sqrt{2}]$.
		\begin{figure}[ht]
			\centering
...
\paragraph{Qual è la relazione tra la Lagrangiana (\ref{eq:lagrangiana}) e Teorema \ref{th:moltiplicatori_lagrange} dei moltiplicatori di Lagrange? (Possibile domanda all'orale)} Tale relazione è dovuta all'equivalenza delle soluzioni di \ref{eq:sistema_lagrangiana_0} e (\ref{eq:identita_vettoriale_moltiplicatore_lagrange}), ovvero: se $(x_0,y_0,\lambda_0)$ è punto stazionario di $L$ allora è soluzione di \ref{eq:sistema_lagrangiana_0} ($L(x_0,y_0,\lambda_0) =\uline 0$) e di (\ref{eq:identita_vettoriale_moltiplicatore_lagrange}).
