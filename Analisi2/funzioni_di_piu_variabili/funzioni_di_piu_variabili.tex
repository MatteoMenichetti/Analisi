...
\begin{enumerate}
	...
	\end{equation*}
	dove, se $\uline h\rightarrow 0$ allora $||\uline h||=0$.
...
\begin{proof}[Dimostrazione (caso $n=2$)]
	...
	\begin{equation}\label{eq:applicazione_lagrange_dim_differenziale}
		\begin{matrix}
			(\ref{eq:differenza_dim_th_differenziale}) &=& \left[\frac{\partial f}{\partial x}(x_1, y + k) (\cancel{x} + h - \cancel{x})\right] + \left[\frac{\partial f}{\partial y}(x,y_1)(\cancel{y} + k -\cancel{y})\right]&=& \uline{f_x(x_1,y+k) h + f_y(x,y_1) k}_{(a)} &=&\\\\
			&& &=& f(x+h,y+k)-f(x,y)\\\\
			&& && \text{con }x_1\underset{h\rightarrow 0}{\longrightarrow}x,\; y_1\underset{k\rightarrow 0}{\longrightarrow}y.
		\end{matrix}
	\end{equation}
	È necessario valutare il rapporto tra due funzioni quando $(h,k)\rightarrow(0,0)$ per concludere la differenziabilità. Dunque, al posto dei due addendi della funzione al numeratore del limite della differenziabilità è sostituito (a) appena trovato. Pertanto, il rapporto da valutare per $(h,k)\rightarrow(0,0)$ è
	...
	Se $(h,k)\rightarrow(0,0)$ allora $x_1\rightarrow x,\, y_1\rightarrow y$, con $x_1\in(x,x+h)$ e $y_1\rightarrow y$, pertanto
	\begin{equation*}
		\begin{matrix}
...
\begin{enumerate}
	...
	\end{aligned}$ è un'applicazione lineare e derivata di $f$ in $x_0$,
...
\begin{theorem}[Regola della catena]\footnote{Slide 3 PDF 18.}
	...
	\begin{equation*}
		\begin{aligned}
			g:[-1,1] &\rightarrow A\subseteq\mathbb{R}^n\\
			t&\mapsto g(t)=(g_1(t)),\hdots,g_n(t)) 
		\end{aligned}
	\end{equation*}
	una curva continua in $[-1,1]$ e derivabile.\\
	...
	\begin{equation*}
		\boldsymbol{F'(0)=}\frac{d(f\circ g)}{dt}(0) = <\triangledown f(\uline x_0, g'(0))>.
	\end{equation*}
\end{theorem}
