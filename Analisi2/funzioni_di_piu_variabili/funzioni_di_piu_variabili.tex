...
\begin{theorem}[di Schwarz]\label{th:schwarz}\footnote{Slide 2 PDF 20.}
	Sia $f:A\subseteq\mathbb{R}^n\rightarrow\mathbb{R}$ e $A$ aperto. Supposto che $f$ sia derivabile due volte in $A$, allora
	\begin{equation*}
		\exists f_{x_ix_j}(\uline x)\quad i,j=1,\hdots, n,\, \forall \uline x\in A\quad (\exists Hf(\uline x),\,\forall \uline x \in A).
	\end{equation*}
	Se le derivate miste $f_{x_ix_j}(\uline x)$ e $f_{x_jx_i}(\uline x)$ sono continue in $\uline x_0\in A$ con $i\neq j$, allora $f_{x_ix_j}(\uline x_0)=f_{x_jx_i}(\uline x_0)$
\end{theorem}
\begin{proof}[Dimostrazione (caso $n=2$)]
	Sia $f:A\subseteq\mathbb{R}^2\rightarrow\mathbb{R},\, P_0=(x_0,y_0)\in A$. Supposto $f$ sia derivabile due volte in $A$, allora
	\begin{equation*}
			\exists Hf(x,y) =
			\begin{bmatrix}
				f_{xx}(\uline x) & f_{xy}(\uline x)\\
	...
	e applicando nuovamente il Teorema di Lagrange a $\frac{\partial f}{\partial x}$ \footnote{$F'(x_1)$ e' considerata allo stesso modo di $F(x)-F(x_0)$ dato che $F'(x_1)=\frac{\partial f}{\partial x}(x_1,y) - \frac{\partial f}{\partial x}(x_1,y_0)$.} nell'intervallo $(y_0,y)$ allora $\exists y_1\in(y_0,y)$ tale che
	\begin{equation*}
	...
	\begin{equation}\label{eq:derivate_dim_schwartz}
		...
		\end{matrix}
	\end{equation}
	Verifichiamo che (le due derivate siano uguali)
	\begin{equation*}
	...
	\end{equation*}
	Si ha dunque (dato che $F(x) - F(x_0) = G(y) - G(y_0)$)
	\begin{equation*}
	...
	\begin{equation*}
		\lim_{(x,y)\rightarrow(0,0)}  \frac{\partial^2 f}{\partial x\partial y}(x,y) = \ \frac{\partial^2 f}{\partial x\partial y}(x_0,y_0) = \frac{\partial^2 f}{\partial y\partial x}(x_0,y_0) =\lim_{(x,y)\rightarrow(0,0)}  \frac{\partial^2 f}{\partial y\partial x}(x,y).
	\end{equation*}
	...
	\begin{equation*}
		...
		\end{matrix}
	\end{equation*}
	Pertanto, facendo il limite delle derivate miste (\ref{eq:derivate_miste_dim_schwarz}) per $(x_1,y_1) \rightarrow (x_0,y_0)$ e $(x_2,y_2)\rightarrow (x_0,y_0)$ è ottenuto che queste sono uguali
	\begin{equation*}
	...
	\end{equation*}
	(con questa uguaglianza ho ciò che serve, le derivate seconde miste sono uguali, cioè (\ref{eq:derivate_miste_dim_schwarz}))
\end{proof}
...
\begin{theorem}[Weierstrass]\label{th:weierstrass_2}\footnote{Slide 21 PDF 23.}
	\textbf{(IPOTESI NECESSARIA)} Ogni funzione $f:K\subset\mathbb{R}^2\rightarrow\mathbb{R}$ continua su $K\subset\mathbb{R}^2$ (compatto perché) limitato e chiuso di $\mathbb{R}^2$ \textbf{(Tesi)} è limitata ed assume su tale insieme i valori di massimo e di minimo assoluti, ovvero $\underline{\exists(x_m,y_m),(x_M, y_M)\in K}_{(a)}$ tc 
	\begin{equation*}
		\underline{f(x_m,y_m)\leq f(x,y)\leq f(x_M, y_M)}_{(b)},\qquad\forall (x,y)\in K.
	\end{equation*}
\end{theorem}
...
\begin{itemize}
	...
	\item Per $(b)$ $f$ è limitata nei punti di minimo e di massimo assoluti $(x_m,y_m)$ e $(x_M, y_M)$,
	\item $f(x_m,y_m)$ e $f(x_M, y_M)$ sono valori di minimo e massimo in $K$ e non punti,
