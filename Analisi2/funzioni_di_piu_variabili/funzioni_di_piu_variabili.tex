...
\begin{example}[Vettore tangente]\footnote{Slide 8 PDF 15.}
 	...
 	\end{aligned}
 	\end{equation}
 	il vettore tangente è $y'(t_0) = (1,f'(t_0))$ (dove la direzione di questo vettore è la direzione della retta tangente).
\end{example}
...
Dato che sono considerate funzioni di $n$ variabili, è conveniente utilizzare la notazione vettoriale per avere una notazione compatta per consentire di non scrivere tutte le componenti di una formula. Per questo è utile ricordare che in $\mathbb{R}^n$ la base canonica forma un sistema fondamentale, cioè ogni elemento di $\mathbb{R}^n$ può essere scritto come combinazione lineare dei vettori della base canonica, dove i vettori della base canonica sono della forma
\begin{equation*}
	\uline e_i=(0,\hdots, 1,\hdots, 0)\in\mathbb{R}^n,\quad i=0,\hdots,n.
\end{equation*}

\noindent Inoltre, $\uline e_i\in\{\uline e_1,\hdots, \uline e_2\}$, elemento della base canonica di $\mathbb{R}^n$, rappresenta un versore della base canonica ed una direzione in $\mathbb{R}^n$.

\paragraph{Esempio di direzione:} $e_1=(1,0)\uline i$ r $e_2=(0,1)\, \uline j$ sono perpendicolari modulo 1 e forniscono le direzione degli assi coordinati (rispettivamente $x$ e $y$).
...
\begin{definition}[Derivata direzionale]\label{def:derivata_direzionale_Rn}\footnote{Slide 11 PDF 15.}
	Sia $f:A\subseteq\mathbb R^n\rightarrow\mathbb R$, con $A$ aperto e sia $\uline x=(x_1,x_2,\hdots, x_n)\in A$. La derivata direzionale di $f$ rispetto alla direzione $\uline e_i$ ed alla variabile $x_i$ (dove $i=1,\hdots, n$) nel punto $\uline x$ è data da
...
\begin{definition}[Derivata direzionale, versione da ricordare]\footnote{Slide 12 PDF 15.}\label{def:derivata_direzionale_Rn_da_ricordare}
	Data $\uline v\in\mathbb{R}^n$, direzione (versore) in $\mathbb{R}^n$, $f:A\subseteq\mathbb R^n\rightarrow\mathbb R$, con $A$ aperto e sia $\uline x=(x_1,x_2,\hdots, x_n)\in A$. La derivata direzionale di $f$ rispetto alla direzione $\uline v$ ed alla variabile $x_i$ (dove $i=1,\hdots, n$) nel punto $\uline x$ è data da
	\begin{equation*}
		D_{\vec{v}} =\frac{\partial f}{\partial \vec{v}}(\uline x) = \lim_{h\rightarrow 0}\frac{f(\uline x + h \uline v) - f(\uline x)}{h},
	\end{equation*}
	\textbf{se il limite esiste ed è finito}.
\end{definition}

\paragraph{Nota sulla Definizione:} $\uline v$ ha modulo unitario, è un versore. In realtà ogni vettore rappresenta una direzione, perché è possibile stabilire modulo unitario e quindi la direzione ed il verso dividendo il vettore per il modulo del vettore stesso.

\begin{example}\footnote{Slide 13 PDF 15.}
	\paragraph{Testo:} Sia $\uline v=(a,b,c)\in\mathbb{R}^3$, dove $a^2+b^2+c^2=1$ (ovvero $||\uline v||=\sqrt{a^2+b^2+c^2}=1$). Data $f(x,y,z)$, calcola-re la sua derivata direzionale in $P_0=(x0,y_0,z_0)$.
...
\begin{example}\footnote{Slide 15 PDF 15.}
	...
	\begin{equation*}
		\begin{matrix}
			\frac{\partial f}{\partial \vec{v}}(0,0) &=& \lim_{t\rightarrow 0}\frac{f(0+tv_1, 0+tv_2) - f(0,0)}{t} &=&\lim_{t\rightarrow 0}\frac{e^{t^3v_1^3}-1}{t^2v_1^2 + t^2v^2_2}\cdot\frac{1}{t} &=&\\
			&=& \lim_{t\rightarrow0}\frac{e^{t^3v^3_1}}{t^2(\underbrace{v_1^2+v_2^2}_{1})}\cdot \frac{1}{t} &=& \lim_{t\rightarrow 0} \frac{e^{t^3v^3_1}}{t^3} = v_1^3\in\mathbb{R}.
		\end{matrix}
	\end{equation*}
...
\begin{enumerate}
	...
	\item (Non ufficiale) $- f(\uline x) - <\triangledown f(\uline x), \uline h>$ è il polinomio di Taylor di $f$ calcolato in $\uline x$ e permette di costruire l'equazione di un piano tangente al grafico di $f$ nel punto $x_0$ $z= f(x_0) + <\triangledown f(\uline x_0), \uline h>$.  Inoltre $\uline h=(x-x_1,\hdots, x-x_n)$, ovvero le variabili sono in funzione degli incrementi, perché la forma del limite (\ref{eq:limite_differenziabilita}) è utilizzata per non cambiare la forma del limite in base al punto in cui vogliamo verificare la differenziabilità, al posto di
	\begin{equation*}
...
\paragraph{Significato geometrico differenziabilità:} Se $\uline x+\uline h$ è pensato come incremento di $\uline x$, è possibile affermare che la differenziabilità di una funzione in un punto ha un significato geometrico, ovvero: la differenziabilità in un punto è legata all'esistenza del piano tangente al grafico della funzione in tale punto. Questo è dovuto al fatto che il piano tangente è un'approssimazione lineare della funzione. Inoltre, $<\triangledown f(\uline x), \uline h>$ in (\ref{eq:limite_differenziabilita}) è una funzione \textbf{\uline{lineare}} del tipo $df(\uline x)(\uline h) = L(\uline h) = <\triangledown f(\uline x), \uline h>$. Pertanto, (\ref{eq:approssimazione_differenziabilita}) risulta essere un'approssimazione lineare opportuna dell'incremento della funzione $f$ ed è per questo che per trattare il piano tangente di $f$ in un punto è necessario richiedere che nel punto valga la differenziabilita di $f$ in tale punto.\\
Quindi, dato quanto scritto per le rette $T1$ e $T2$ nel piano  ottenute considerando le sezioni trasversali. $\qed$

\paragraph{Derivabilità $\neq$ continuità:} Come sarà visto in Sezione \ref{sssec:differenziabilita_continuita}, la derivabilità non implica (come per le funzioni di una variabile) la continuità e l'esistenza del piano tangente. Per una funzione di una varibile, la derivabilità implica l'esistenza della retta tangente.$\qed$
\vspace{2px}
...
\begin{equation*}
	(\ref{eq:approssimazione_differenziabilita}) \equiv f(\uline x_0) +   <\triangledown f(\uline x_0), \uline h> + o(||\uline h||) \quad \text{per}\quad \uline h\rightarrow \uline 0,
\end{equation*}
...
\end{enumerate}} e dunque la funzione lineare (la quale dipende solo da $\uline x$ perché $\uline x_0$ è fissato)
...
\paragraph{Nota su (\ref{eq:piano_tangente_grafico_due_variabili}):} (\ref{eq:piano_tangente_grafico_due_variabili}) è un'espressione lineare in $x,y$ in forma implicita del tipo $a(x) + b(y) + c(z) = d$. $\qed$
...
\end{equation}
dove $\sqrt{(x-x_0)^2 + (y-y_0)^2} = d(P,P_0) = ||P-P_0||$, $P=(x,y),P_0=(x_0,y_0)\in\mathbb{R}^2$.
...
\begin{equation}\label{eq:uguaglianza_piano}
	\underbrace{f(x,y)  -  f(x_0, y_0) - \frac{\partial f}{\partial x} (x_0, y_0)(x-x_0) - \frac{\partial f}{\partial y} (x_0,y_0) (y-y_0))}_{\text{(b)}}=\underbrace{o\left(\underbrace{\sqrt{(x-x_0)^2 + (y-y_0)^2}}_{\text{(c)}}\right)}_{\text{(a)}}\quad\text{con}\; (x,y)\rightarrow(x_0,y_0),
\end{equation}
...
\footnotemark{Il nominatore è un $o$-piccolo del denominatore.}

\paragraph{Osservazione su $\boldsymbol{o}$-piccolo in (\ref{eq:uguaglianza_piano}):} L'errore commesso nell'approssimazione di $f$ con il piano $L$ è (a) in (\ref{eq:uguaglianza_piano}), ovvero l'$o$-piccolo della distanza $||P-P_0||$. Quindi l'errore (a) in (\ref{eq:uguaglianza_piano}) è in notazione simile a quella utilizzata con Taylor e (\ref{eq:intro_o_piccolo}) può essere scritta come
\begin{equation*}
	f(x,y) = L(x,y) + o(||P-P_0||).
\end{equation*}
...
\begin{property}[Non ufficiale]\footnote{Slide 8, PDF 16.}
	Se $\underbrace{f\text{ è differenziabile}}_{\text{Definizione \ref{def:differenziabilita_funzione_piu_variabili}}}\rightarrow\quad\underbrace{f \text{ derivabile}}_{\text{in } \uline x_0 \text{ se } \exists\triangledown f(\uline x_0)}\rightarrow\quad \underbrace{f \text{ continua}}_{\text{se } \lim_{\uline x_0\rightarrow\uline x_0}f(\uline x)=f(\uline x_0)}$. 
\end{property}
...
\begin{proof}
	...
	\end{equation*}
	Valutiamo dunque (il limite per un incremento nella direzione $\uline h$)
	\begin{equation*}
		\lim_{\uline h\rightarrow \uline 0} f(\uline x_0 + \uline h) = \lim_{\uline h\rightarrow \uline 0} = [\text{nella dimostrazione scrivere }\footnotemark] = \lim_{\uline h\rightarrow\uline 0} \left[f(x_0) + <\triangledown f(\uline x_0),\, \uline  h> + o(||\uline h||)\right].
	\end{equation*}
	Sia \footnotetext{Dato che $f$ è differenziabile in $\uline x_0$ allora è possibile scrivere
		\begin{equation*}
			f(\uline x_0+\uline h)=f(\uline x_0) + <\triangledown f(\uline x_0),\, \uline h> + o(||\uline h||),\quad\text{per }\uline h\rightarrow\uline 0,
		\end{equation*}
		e sostituendo è ottenuto quanto segue alla nota.}
	\begin{equation*}
	...
	\begin{equation}\label{eq:limite_f(x0)_uguale_f(x0)}
			\lim_{\uline h\rightarrow \uline 0} f(\uline x_0 + \uline h) = 	\lim_{\uline x\rightarrow \uline x_0} f(\uline x_0) = f(\uline x_0).
	\end{equation}
\end{proof}

\paragraph{Nota:} Se $f$ è differenziabile in $x_0$ allora $f$ è derivabile e continua in $x_0$.

\begin{theorem}[del differenziale]\label{th:del_differenziale}\footnote{Slide 9 PDF 17}
	Sia $f:A\subseteq\mathbb{R}^n\rightarrow\mathbb{R}$, con $A$ aperto. \textbf{(Ipotesi)} Sia $f$ derivabile in $A$ (ovvero $\exists\triangledown f(\uline x),\, \forall\uline x\in A$). Se le derivate parziali $f_{x_1}(\uline x),\hdots, f_{x_n}(\uline x)$, sono continue in $\uline x$ allora \textbf{(tesi)} $f$ è differenziabile in $\uline x\in A$.
\end{theorem}
\begin{proof}[Dimostrazione (caso $n=2$)]
	...
	\begin{equation}\label{eq:differenza_dim_th_differenziale}
		\begin{matrix}
			f(h+h,y+k) - f(x,y) &\underset{+f(x,y+k)-f(x,y+k)}{\overset{\text{aggiungo}}{=}}& f(\uline x + \uline h) + f(x,y+k) - f(x,y+k) - f(x,y) &=&\\
