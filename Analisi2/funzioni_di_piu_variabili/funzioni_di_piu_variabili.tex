...
\paragraph{Osservazione sulla condizione \ref{eq:limite_differenziabilita} della precedente definizione:}
\begin{enumerate}
	\item (Ufficiale)
	\begin{equation*}
		<\triangledown f(\uline x), \uline h> = \sum_{i=1}^{n} \frac{\partial f}{\partial x_i}(\uline x)h_i = \frac{\partial f}{\partial x_1}(\uline x)h_1 + \hdots + \frac{\partial f}{\partial x_n}(\uline x)h_n,
	\end{equation*}
	con
	\begin{equation*}
...
\begin{definition}[Differenziale/funzionale]\footnote{Slide 2 PDF 16.}
	Fissato $x\in A$, punto nel quale $f:A\rightarrow\mathbb{R}$ è differenziabile, l'applicazione (funzionale) lineare$_{\footnotemark}$
	\begin{equation*}
		\begin{aligned}
...
\begin{definition}[Funzione differenziabile (alternativa)]\label{def:funzione_differenziabile_alternativa} \footnote{Slide 2 PDF 16.}
	Sia $f:A\rightarrow\mathbb{R}$, dove $A$ è aperto, $f$ si dice differeziabile in $\uline x\in A$ se esiste un funzionale lineare
	\begin{equation*}
		\begin{aligned}
			L:\mathbb{R}^n&\rightarrow\mathbb{R}\\
			\uline h&\mapsto L(\uline h) = <\triangledown f(\uline x), \uline h>
...
\begin{definition}[Piano tangente al grafico di una funzione (con $n=2$)]\footnote{Slide 5 PDF 16.}
	Sia $f:A\subseteq \mathbb{R}^2\rightarrow\mathbb{R}$, con $P=(x_0,y_0)\in A$. È supposto che $f$ sia differenziabile in $P_0$. L'equazione del piano tangente al grafico nel punto $P_0$ è data da
...
\paragraph{Nota su (\ref{eq:piano_tangente_grafico_due_variabili}):} (\ref{eq:piano_tangente_grafico_due_variabili}) è un'espressione lineare in $x,y$ in forma implicita del tipo $a(x) + b(y) + c(z) = d$.
