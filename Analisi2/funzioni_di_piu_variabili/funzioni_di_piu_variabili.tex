...
\paragraph{Osservazione sulla condizione 2. della precedente definizione:}
\begin{enumerate}
	\item (Ufficiale)
	\begin{equation*}
	...
	\item (Non ufficiale) $- f(\uline x) - <\triangledown f(\uline x), \uline h>$ è il polinomio di Taylor di $f$ calcolato in $\uline x$ e permette di costruire l'equazione di un piano $z= f(x_0) + <\triangledown f(\uline x), \uline h>$.  Inoltre $\uline h=(x-x_1,\hdots, x-x_n)$, ovvero le variabili sono in funzione degli incrementi, perché la forma del limite (\ref{eq:limite_differenziabilita}) è utilizzata per non cambiare la forma del limite in base al punto in cui vogliamo verificare la differenziabilità, al posto di
	\begin{equation*}
