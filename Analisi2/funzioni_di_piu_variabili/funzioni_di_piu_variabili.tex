...
\begin{proof}[Dimostrazione (caso $n=2$)]
	...
	\footnote{$F$ e $G$ sono funzioni di una variabile. Applicando il Teorema di Lagrange saranno considerate le derivate parziali di $f$.} Considerato $F(x)$ è applicato il Teorema \ref{th:lagrange_analisi1} di Lagrange nell'intervallo di estremi $x_0$ e $x$ \footnote{In Figura \ref{fig:dimostrazione_schwarz}, è considerato $[x_0,x]$ per $f$ e $(x_0, x)$ per $f'$.} allora $\exists x_1\in(x_0,x)$ tale che
	\begin{equation*}
		F(x) - F(x_0) = F'(x_1) (x - x_1) \overset{\footnotemark}{=} \underbrace{\left[\frac{\partial f}{\partial x}(x_1, y)- \frac{\partial f}{\partial x}(x_1, y_0)\right]}_{\footnotemark}(x-x_0) \overset{\footnotemark}{=} (\star)
	\end{equation*}
	\addtocounter{footnote}{-2}
	...
	applicando il Teorema di Lagrange a $\frac{\partial f}{\partial x}$ nell'intervallo $(y_0,y)$, quindi $\exists y_1\in(y_0,y)$ tale che
	\begin{equation*}
		(\star) = \frac{\partial^2 f}{\partial x\partial y}(x_1,y_1)\underbrace{(y-y_0)(x-x_0)}_{\neq 0}.
	\end{equation*}
	
	Ripetiamo analogo ragionamento per $G(y)$, cioè è applicato il Teorema di Lagrange nell'intervallo di estremi $y$ e $y_0$, quindi $\exists y_2\in\uline{(y_0,y)}_{\footnotemark}$ : \footnotetext{$(y_0,y)$ perché riportato il caso di Figura \ref{fig:dimostrazione_schwarz}, altrimenti $(y,y_0)$.}
	\begin{equation*}
	...
	e
	\begin{equation*}
		G(y) - G(y_0) = \overbrace{f(x,y) - f(x, y_0)}^{G(x)} + \overbrace{f(x_0,y) - f(x_0,y_0)}^{G(x_0)} = f(x,y) - f(x_0, y) - f(x, y_0) + f(x_0,y_0).
	\end{equation*}
	(Quindi $F(x) - F(x_0) = G(y) - G(y_0)$)
	Si ha dunque
	\begin{equation*}
...
\paragraph{IMPORTANTE:} \textbf{Il Test dell'Hessiana può essere utilizzato SE E SOLO SE} $\boldsymbol{\det(Hf(x_0,y_0))\neq 0}$. Se $\det (Hf(x_0,y_0)) = 0$ è necessario utilizzare un altro metodo per stabilire la natura del punto critico e ciò è fatto tramite il calcolo della variazione della funzione nei punti vicino al punto critico.

\paragraph{Note su 1. e 2.:} Quando è stata introdotta la formula di Taylor è stato mostrato che il polinomio è omogeneo in $h$ e $k$ i cui coefficienti sono la derivata seconda. Pertanto, quando si ha una matrice simmetrica (come $Hf(\uline x)$ quando $f\in C^1(A)$) è possibile associare a questa una forma quadratica. Tale forma quadratica ha un segno e quindi fornisce informazioni riguardo massimo e minimo relativo.

\paragraph{Se $\boldsymbol{\det(Hf(x_0,y_0))=0}$?} Non è possibile applicare il Test dell'Hessiana ma è necessario valutare il segno di $f(\uline x) - f(\uline x_0)$ in un intorno qualsiasi di $(x_0,y_0)$ (come negli Esercizi \ref{ex:riferimento_test_hessiana}).
...
Alla dimostrazione del test dell'Hessiana (\ul{non sarà fatta}) è associata la seguente approssimazione tramite la formula di Taylor con resto di Peano
...
(\ref{eq:approssimazione_taylor_resto_peano_dim_test_hessiana}) può essere riscritta in forma vettoriale come segue
\begin{equation*}
...
\begin{definition}[Forma quadratica]\footnote{Slide 15 PDF 23.}
	...
	dove
	\begin{equation*}
		q (\uline h) = a_{11}\uline h_1^2 + \rightarrow a_{22}\uline h_2^2 + \hdots + a_{nn}\uline h_n^2 + \sum_{i,j=1, i\neq j}^{n} 2a_{ij} h_ih_j = \sum_{i,j=1}^{n} a_{ij}h_i h_j
	\end{equation*}
...
\begin{definition}[Definizione del forma quadratica, non ufficiale]\footnote{Slide 17 PDF 23.}
	$q:\mathbb{R}^n\rightarrow\mathbb{R}$ (o la sua matrice associata) si dice
	\begin{itemize}
...
$g(x,y)=k$ in (\ref{eq:vincolo_funzione}) è un insime di livello $k$ della funzione $g$. Tale insieme è un sottoinsime di $\mathbb{R}^2$ (nel nostro caso di $A$ perché $f$ è considerata in un dominio). Peranto, (\ref{eq:vincolo_funzione}) è un insieme di livello.
...
\end{equation*}
riducendo così il problema ad una variabile. (Un esempio di parametrizzazione è l'esempio a SLIDE 25 PDF 23 PARAMETRIZZAZIONE $f$ SU VINCOLO $V$)

Inoltre, è possibile esplicitare variabili in funzione delle altre. Un esempio: $y=y(x),\, f_{|_V}(x,y) = f(x, y(x))$, la quale è una funzione di una variabile.
