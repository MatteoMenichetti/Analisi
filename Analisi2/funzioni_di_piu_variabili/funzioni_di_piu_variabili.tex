...
\paragraph{Cosa serve per al Teorema di Schwarz per valere?} Le ipotesi del Teorema richiedono l'esistenza e la continuità delle funzioni in $A$ (ovvero che $f\in C^2(A)$). La sola esistenza delle derivate seconde non assicura che si possa invertire l'ordine dei derivazione. Ciò è fatto vedere nel seguente esempio.
...
\begin{definition}[Minimo e massimo locale]\label{def:minimo_massimo_locale_an2}\footnote{Slide 3 PDF 22.}
	Sia $f:A\subset\mathbb{R}^n\rightarrow\mathbb{R}$ (nel caso generale, al posto di $A$ c'è $D=A\cup\partial A$, dove $A=\mathring D$). Si dice che $\uline x_0\in A$ ($\slash\in D$) è un minimo relativo (locale) per $f$ se, per $r>0$
	\begin{equation*}
...
\begin{definition}[Minimo e massimo assoluto (globale)]\footnote{Slide 3 PDF 22.}
...
\begin{theorem}[di Fermat]\label{th:fermat_an2}\footnote{Slide 5 PDF 22.}
	Sia $f:A\subseteq\mathbb{R}^n\rightarrow\mathbb{R}$ ($D$ nel caso generale), $A$ aperto e sia $\uline x_0\in A$ ($\slash \uline x_0\in \mathring D$) punto di minimo (massimo) relativo (locale).\\
...
\begin{remark}[Non ufficiale]\footnote{Slide 6 PDF 22.}
...
\paragraph{A cosa serve la differenziabilità nel Teorema di Fermat?}  Per la regola della catena e le proprietà che ne derivano. Se per una funzione di una variabile è richiesto che $f$ sia derivabile affinché sia continua, in più variabili è richiesta la differenziabilità.
...
I punti che annullano il gradiente ma che non sono di minimo o di massimo si chiamano di sella (o di colle). Ciò significa che nell'intorno del punto il comportamento (il segno) della funzione $f(\uline x) - f(\uline x_0)$ cambia a seconda di come varia $\uline x$. 
...
\paragraph{Nota:} Il segno di $f(\uline x) - f(\uline x_0)$ determina se $\uline x_0$ è di minimo o di massimo relativo, ovvero data Definizione \ref{def:minimo_massimo_locale_an2} di minimo e massimo relativo vale la seguente Osservazione.
...
\begin{definition}[Test dell'Hessiana]\label{def:test_hessiana}\footnote{Slide 12 PDF 22.}
	\textbf{(Ipotesi)} Sia $f:A\subset \mathbb{R}^2\rightarrow\mathbb{R}$, $A$ aperto con $f\in C^2(A)$ (quindi $Hf(x,y)$ è simmetrica) e sia $(x_0,y_0)\in A$ punto critico (stazionario) per $f$ e tc $\boldsymbol{\uline{\det(Hf(x_0,y_0))\neq 0}}$, dove
	\begin{equation*}
		\begin{matrix}
			\det(Hf(x_0,y_0)) &=&
