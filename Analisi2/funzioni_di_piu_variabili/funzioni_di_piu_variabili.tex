...
Tornando alla Definizione di differenziabilità, se consideriamo $\uline h\rightarrow \uline0$, dunque $||\uline h||\rightarrow 0$ e quindi
\begin{equation*}
...
\end{equation}
e dunque (a partire (\ref{eq:limite_differenziabilita}) è ottenuto)
...
\paragraph{Significato geometrico differenziabilità:} Se $\uline x+\uline h$ è pensato come incremento di $\uline x$, è possibile affermare che la differenziabilità di una funzione in un punto ha un significato geometrico, ovvero: la differenziabilità in un punto è legata all'esistenza del piano tangente al grafico della funzione in tale punto. Questo è dovuto al fatto che il piano tangente è un'approssimazione lineare della funzione. Inoltre, $<\triangledown f(\uline x), \uline h>$ in (\ref{eq:limite_differenziabilita}) è una \uline{funzione \textbf{lineare}} del tipo $df(\uline x)(\uline h) = L(\uline h) = <\triangledown f(\uline x), \uline h>$. Pertanto, (\ref{eq:approssimazione_differenziabilita}) risulta essere un'approssimazione lineare opportuna dell'incremento della funzione $f$ ed è per questo che per trattare il piano tangente di $f$ in un punto è necessario richiedere che nel punto valga la differenziabilità di $f$ in tale punto.\\
...
\begin{proof}[Dimostrazione (caso $n=2$)]
	Sia $f:A\subseteq \mathbb{R}^2\rightarrow\mathbb{R}$, con $A$ aperto. Dati $\uline h = (h,k)\in\mathbb{R}^2,\, \uline x=(x,y)\in A$, \footnote{È necessario verificare che il limite per $\uline h\rightarrow\uline 0$ del rapporto incrementale di $f$ esiste ed è 0 e per farlo è scritto in modo opportuno parte del rapporto.}
	\begin{equation}\label{eq:differenza_dim_th_differenziale}
		...
		\end{matrix}
	\end{equation}
	applicando il Teorema \ref{th:lagrange_analisi1} di Lagrange ad $f(x,y)$ (supponendo $h,k>0$) negli intervalli $(x,x+h)$\footnotemark e $(y,y+k)$\footnotemark, allora $\exists x_1\in(x,x+h)$ e $\exists y_1\in(y,y+k)$ tale che
	\begin{equation}\label{eq:applicazione_lagrange_dim_differenziale}
		...
		\end{matrix}
	\end{equation}
	È necessario valutare il rapporto tra due funzioni quando $(h,k)\rightarrow(0,0)$ per concludere la differenziabilità. Dunque, al posto dei due addendi della funzione al numeratore del limite della differenziabilità è sostituito (a) in (\ref{eq:applicazione_lagrange_dim_differenziale}). Pertanto, per stabilire la differenziabilità è necessario valutare il limite del seguente rapporto per $(h,k)\rightarrow(0,0)$ sia 0
	\begin{equation}\label{eq:rapporto_differenziabilita_dim_differenziale}
		\frac{f(x+h,y+k)-f(x,y)\overbrace{-f_x(x,y)h-f(x,y)k}^{-<\triangledown f(\uline x),\uline h>}}{\sqrt{h^2+y^2}}.
	\end{equation}
	Consideriamo dunque quanto segue per applicare il Teorema del Confronto
	...
	Se $(h,k)\rightarrow(0,0)$ allora $x_1\rightarrow x,\, y_1\rightarrow y$, con $x_1\in(x,x+h)$ e $y_1\in(y, y+k)$, pertanto
	\begin{equation*}
		\begin{matrix}
	...
	\begin{equation*}
		...
		\end{matrix}
	\end{equation*}
	Pertanto, utilizzando il teorema del confronto, è stato dimostrato che (\ref{eq:rapporto_differenziabilita_dim_differenziale})$\underset{(h,k)\rightarrow(0,0)}{\longrightarrow}0$, quindi la differenziabilità di $f$.
\end{proof}
