...
\begin{theorem}[di Schwarz]\label{th:di_schwarz}\footnote{Slide 2 PDF 20.}
...
\begin{proof}[Dimostrazione (caso $n=2$)]
	...
	Sia $P=(x,y)\in A$, con $P\neq P_0$ ($P$ punto qualsiasi, in questo caso scelto maggiore di $P_0$ e libero di muoversi fissata una variabile).\\
	Definiamo (due funzioni per verificare le HP)
	\begin{equation*}
		\begin{matrix}
		...
		\end{matrix}
	\end{equation*}
	\footnote{$F$ e $G$ sono funzioni di una variabile. Applicando il Teorema di Lagrange saranno considerate le derivate parziali di $f$.} Considerato $F(x)$ è applicato il Teorema di Lagrange nell'intervallo di estremi $x_0$ e $x$ \footnote{In Figura \ref{fig:dimostrazione_schwarz}, è considerato $[x_0,x]$ per $f$ e $(x_0, x)$ per $f'$.} allora $\exists x_1\in(x_0,x)$ tale che
	\begin{equation*}
	...
	\stepcounter{footnote}
	\footnotetext{Dato che $f\in C^2(A)$ allora $\frac{\partial f}{\partial y}$ è derivabile ulteriormente rispetto $x$ e quindi è applicabile nuovamente il Teorema di Lagrange.}
...
\begin{itemize}
	...
	\begin{equation*}
		Hf(x,y) =
		\begin{bmatrix}
			\frac{\partial^2 f}{\partial x^2} & &	\boxed{\frac{\partial ^2 f}{\partial x\partial y}}\\
			&\horeq&\\
			\boxed{\frac{\partial ^2 f}{\partial y\partial x}} && \frac{\partial^2 f}{\partial y^2}
...
\paragraph{Cos'è un segmento?} È l'insieme di punti che congiunge $\uline x$ a $\uline x + \uline h$ lungo la direzione $\uline h$ al variare di $t$.

\paragraph{Nota:} Presi due punti in $A$, la congiunzione di questi due è il movimento dal primo al secondo punto lungo la direzione indicata $\uline h$. Ciò fornirà una curva in $A$ perché calcolato cosa accade lungo la curva su $[\uline x, \uline x+\uline h]$ tramite $\uline x(t)$. $\qed$
...
\begin{property}[Proprietà indotte da $f$ a $F$, non ufficiali]\label{eq:proprieta_indotte_f_F}
	...
	Supposto $f\in C^2(A)$ allora $F\in C^2([0,1])$, $F'(t)$ è ulteriormente derivabile come
	\begin{equation}\label{eq:regola_catena_funzione_composta_curva_2}
		F''(t) = (F'(t))' \overset{\footnotemark}{=} \sum_{j=1}^{n}\frac{\partial f}{\partial x_i}  (\sum_{i=1}^{n}\frac{\partial f}{\partial x_i} (\uline x + t \uline h)\, h_i)h_j = \uline{\sum_{i,j=1}^{n} \frac{\partial^2 f}{\partial x_i} (\uline x + t \uline h)\, h_ih_j}_{(a)} [= <Hf(\uline x + t\uline h)\uline h, \uline h>],
	\end{equation}
...
Sia la Definizione \ref{eq:funzione_composta_curva}, ovvero
$f:A\subseteq\mathbb{R}^n\rightarrow\mathbb{R},\, f\in C^k(A)$ e $\uline x, \uline x+\uline h\in A$ con $\uline h\neq\uline 0$, tale che $[\uline x, \uline x+\uline h] = \{x(t)\in\mathbb{R}^n; \uline x(t) = \uline x + t\uline h,\, t\in[0,1]\}\subset A$, $F(t)=f(\uline x(t))=f(\uline x+t\uline h)$ e le Proprietà \ref{eq:proprieta_indotte_f_F}, è possibile dare la definizione di formula di Taylor (opportunamente centrato) con resto secondo Lagrange tramite la definizione per le funzioni di una variabile reale.

\begin{definition}[Polinomio di Taylor di grado $n$ centrato in 0 con resto secondo Lagrange]\footnote{Slide 23 PDF 21.}
	Sia $f\in C^k(A)$ allora $F\in C^k([0,1])$ con $F[0,1]\rightarrow\mathbb{R}$, si ha allora che $\exists\theta\in(0,1)$ tale che
	\begin{equation*}
		F(\underbrace{1}_t) = F(0) + \frac{F'(0)}{1}\cdot \underbrace{1}_t
	\end{equation*}
\end{definition}