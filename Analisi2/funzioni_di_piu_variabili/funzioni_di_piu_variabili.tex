...
\begin{theorem}[Weiestrass]\footnote{Slide 21 PDF 23.}
...
\paragraph{Note sul Teorema di Weiestrass:}
\begin{itemize}
...
\begin{equation*}
	f(\uline x_0 + \uline h) - f(\uline x_0) = \frac{1}{2}\underbrace{<Hf(\uline x_0)\uline h, \uline h>}_{q(\uline h)} + o(||h||^2)\quad \text{per }\uline h\rightarrow\uline 0.
\end{equation*}

Quindi, scegliendo in modo opportuno la direzione $\uline h$ allora il segno di $f(\uline x_0 + \uline h) - f(\uline x_0)$ è deciso da $<Hf(\uline x_0)\uline h, \uline h>$.

Inoltre, è necessario chiarire che $<Hf(\uline x_0)\uline h, \uline h> = \uline h^T Hf(x_0,y_0)\uline h = q(\uline h)$ è la forma quadratica associata a $Hf(x_0,y_0)$ ed è un polinomio omogeneo di grado due nelle variabili $h_1,\hdots, h_n$ (in questo caso $n=2$).

\subsubsection{Forma quadratica}
Il segno della forma quadratica è importante per stabilire la natura di un punto critico. In mancanza di un metodo alternativo, per determinare la natura di un punto critico è quello di utilizzare la formula di Taylor per analizzare il segno dell'incremento $\Delta f(\uline x_0) = f(\uline x_0 + \uline h) - f(\uline x_0)$. Per questo è enunciato il Teorema \ref{th:segno_forma_quadratica} del segno della forma quadratica.
...
\begin{theorem}\label{th:segno_forma_quadratica}
