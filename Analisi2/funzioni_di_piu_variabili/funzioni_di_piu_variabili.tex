...
\begin{definition}[Minimo e massimo locale]\label{def:minimo_massimo_locale_an2}\footnote{Slide 3 PDF 22.}
	Sia $f:A\subset\mathbb{R}^n\rightarrow\mathbb{R}$ \footnote{Nel caso generale, al posto di $A$ c'è $D=A\cup\partial A$, dove $A=\mathring D$.}. Si dice che $\uline x_0\in A$ ($\backslash\in D$) è un minimo relativo (locale) per $f$ se, per $r>0$
	\begin{equation*}
...
\begin{definition}[Minimo e massimo assoluto (globale)]\footnote{Slide 3 PDF 22.}
	...
	\begin{equation*}
		f(\uline x)\leq f(\uline x_0) \quad \forall\uline x\in B(\uline x_0,r)\cap A.
	\end{equation*}
\end{definition}

\begin{theorem}[Weierstrass]\label{th:weierstrass_2}\footnote{Slide 21 PDF 23.}
	\textbf{(IPOTESI NECESSARIA)} Ogni funzione $f:K\subset\mathbb{R}^2\rightarrow\mathbb{R}$ continua su $K\subset\mathbb{R}^2$ (compatto perché) limitato e chiuso di $\mathbb{R}^2$ \textbf{(Tesi)} è limitata ed assume su tale insieme i valori di massimo e di mino assoluti, ovvero $\underline{\exists(x_m,y_m),(x_M, y_M)\in K}_{(a)}$ tc 
	\begin{equation*}
...
\begin{theorem}[di Fermat]\label{th:fermat_an2}\footnote{Slide 5 PDF 22.}
	Sia $f:A\subseteq\mathbb{R}^n\rightarrow\mathbb{R}$ ($D$ nel caso generale), $A$ aperto e sia $\uline x_0\in A$ ($\backslash \uline x_0\in \mathring D$) punto di minimo (massimo) relativo (locale).\\
...
\begin{proof}
	...
	\begin{equation*}
		...
		\end{aligned}
	\end{equation*}
	\footnotemark funzione di una variabile, dove in $t=0$ ho un punto di massimo relativo (perché è ottenuto $\uline x_0$ e questo è il massimo imposto all'inizio) e
	\begin{equation*}
		F(0) = f(\uline x_0).
	\end{equation*}
	\footnotemark Applichiamo il Teorema di Fermat a $F(t)$, dove $t\in(-r,r)$, (dato che $F$ è derivabile in $t=0$)
	\begin{equation*}
		F'(0) = 0.
	\end{equation*}
	(in quanto in $t=0$ ha il massimo) Per la regola della catena
	\begin{equation*}
...
\stepcounter{footnote}
\footnotetext{$F$ è definita su $(-r,r)=I(0,r)\subset\mathbb{R}$, con $r>0$ perché $\uline x_0$ è un estremo locale e quindi vale per un intorno di $t=0$. È scelto $t=0$ ed un intorno di 0 per $t$ perché così $\uline x_0 + t\uline v=\uline x_0$ punto di massimo.}
...
\begin{remark}[Non ufficiale]\footnote{Slide 6 PDF 22.}
	Il Teorema \ref{th:fermat_an2} di Fermat è la \textbf{condizione necessaria} affinché un punto interno al dominio $\uline x_0\in\mathring D$ ($\uline x_0\in A$) sia un estremo relativo della funzione $f\in C^2(\mathring D)$ (ovvero $f\in C^1(A)$). Quindi
	\begin{equation*}
		\begin{matrix}
...
\begin{definition}[Test dell'Hessiana]\label{def:test_hessiana}\footnote{Slide 12 PDF 22.}
	\textbf{(Ipotesi)} Sia $f:A\subset \mathbb{R}^2\rightarrow\mathbb{R}$, $A$ aperto con $f\in C^2(A)$ (quindi $Hf(x,y)$ è simmetrica) e sia $(x_0,y_0)\in A$ punto critico (stazionario) per $f$ e tc $\det(Hf(x_0,y_0))\neq 0$, dove
	\begin{equation*}
		\begin{matrix}
			\det(Hf(x_0,y_0)) &=&
...
\paragraph{Se $\det(Hf(x_0,y_0))=0$?} Non è possibile applicare il Test dell'Hessiana ma è necessario valutare il segno di $f(\uline x) - f(\uline x_0)$ in un intorno qualsiasi di $(x_0,y_0)$ (come negli Esercizi \ref{ex:riferimento_test_hessiana}).

\paragraph{Note su $Hf(x_0,y_0)$:} Dato che $Hf(x_0,y_0)$ è simmetrica allora i suoi autovalori sono soluzione dell'equazione (di secondo grado) agli autovalori \footnote{L'equazione agli autovalori è $\lambda^2 + \lambda s + p = 0$.}
\begin{equation*}
...
\begin{itemize}
	...
	\item $(a)=<Hf(x_0,y_0)\uline h,\uline h>$, con $\uline h=(h,k)$, $h=x-x_0$ e $k=y-y_0$, è un forma quadratica in $\uline h$, un polinomio omogeneo di grado 2 in $(h,k)$, in cui i coefficienti sono dati dalle derivate seconde,
...
\begin{definition}[Forma quadratica]\footnote{Slide 15 PDF 23.}
	...
	\begin{equation}\label{eq:forma_quadratica}
		\begin{aligned}
			...
			q(\uline h) &\rightarrow a_{11}\uline h_1^2 + \rightarrow a_{22}\uline h_2^2 + \hdots + \rightarrow a_{nn}\uline h_n^2 + \sum_{i,j=1, i=j}^{n} 2a_{ij} h_ih_j = \sum_{i,j=1}^{n} a_{ij}h_i h_j
		\end{aligned}
	\end{equation}
	dove i coefficienti reali $a_{ij}$ individuano la matrice simmetrica $A=(a_{ij})$ associata a $q$.
\end{definition}
...
\paragraph{DA RICORDARE:} Ad ogni forma quadratica è associata in modo univoco una matrice simmetrica.
...
\begin{example}[Forma quadratica, caso $n=2$]\footnote{Slide 16 PDF 23.}
	...
	\end{equation*}
	\paragraph{Perché $\lambda_1$ e $\lambda_2$?} $\lambda_1$ e $\lambda_2$ sono gli autovalori di $A$ e quindi le soluzioni dell'equazione caratteristica
	\begin{equation*}
...
\begin{theorem}[Studio del segno della forma quadratica in due variabili, non ufficiale]\label{th:studio_segno_forma_quadratica}\footnote{Slide 16 PDF 23.}
	...
	\begin{enumerate}
		\item se $\boldsymbol{\det A>0}$ e $ \boldsymbol{\lambda_1\lambda_2>0}$ allora $\boldsymbol q$ è definita positiva (e quindi $\boldsymbol{\uline x_0}$ è un \textbf{minimo locale}),
		\item se $\boldsymbol{\det A>0}$ e $ \boldsymbol{\lambda_1\lambda_2<0}$ allora $\boldsymbol q$ è definita negativa (e quindi $\boldsymbol{\uline x_0}$ è un \textbf{massimo locale}),
...
\begin{definition}[Definizione del forma quadratica, non ufficiale]\footnote{Slide 17 PDF 23.}
	...
	\begin{itemize}
		...
		indefinita (se cambia di segno, quindi non è possibile assegnargliene uno, ovvero) se $\exists \uline h_1,\uline h_2\neq \uline 0$ tc $q(\uline h_1)<0<q(\uline h_2)$ (sse gli autovalori di $A=(a_{ij})$ sono positivi e negativi)
	\end{itemize}
\end{definition}
...
Determinare gli estremi (massimo/minimo) vincolati di $f$ significa ricercare i punto $(x,y)\in A$ tali che questi verifichino la condizione aggiuntiva (espressa dal vincolo) della forma $g(x,y)=k$ (assimilabile all'insieme di livello), dove $g\in C^1(A)$.

In Analisi 2 possono essere considerate funzioni in $n$ variabili con $m$ a condizione che $m<n$. Quindi, nel nostro caso, con $n=2$, i punti del piano per i quali $f$ è ben definita, verificano un solo vincolo.
...
Quando i precedenti metodi non funzionano è applicato il Teorema dei Moltiplicatori di Lagrange per determinare minimo e massimo assoluto su $A$.
...
\paragraph{Cosa fa il Teorema dei moltiplicatori di Lagrange?} Fornisce un'equazione vettoriale che permette di ricercare punti di minimo e di massimo per una funzione detta Lagrangiana, la quale è una funzione di tre variabili: le due variabili di $f$ e $\lambda$, il moltiplicatore del problema.

\begin{theorem}[dei moltiplicatori di Lagrange]\label{th:moltiplicatori_lagrange}\footnote{Slide 2 PDF 24.}
	Siano $f,g\in C^1(A)$, dove $A\subseteq\mathbb{R}^2$ aperto. \textbf{(Ipotesi)} Se $(x_0,y_0)\in V$ è un punto di estremo (minimo/massimo) per $f$ nell'insieme (di livello $k$ per $g$) $V=\{(x,y)\in A; g(x,y)= k\}$ e se $\triangledown g(x_0,y_0)\neq 0$ \textbf{\ul{allora} (tesi)} $\exists \lambda_0\in\mathbb{R}$ tale che
	\begin{equation*}
