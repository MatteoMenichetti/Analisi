...
\begin{definition}[Norma euclidea]
    \begin{equation}
        \boldsymbol{||\uline x||_2}=|\uline x|:=\sqrt{<\uline x,\uline x>}=\boldsymbol{\sqrt{\sum_{i=1}^n x_i^2}}\in\mathbb R_0^+,\quad \forall x\in\mathbb R^n.
    \end{equation}
\end{definition}
...
\begin{definition}[Spazio normato]\label{def:spazio_normato}\footnote{Definizione non fatta.}
    Uno \gls{spazio lineare} $X\subseteq \mathbb R^n$ si dice normato se è definita una norma (un'applicazione) $||\cdot||\colon X\rightarrow\mathbb R$.
\end{definition}
...
\begin{definition}[Punto di accumulazione]\footnote{Slide 2 PDF 9.}
    ...
    \begin{equation*}
        \forall r>0,\quad B(\uline x_0, r)\cap (A\backslash\{x_0\})\neq\emptyset.
    \end{equation*}
\end{definition}
...
\begin{definition}[Funzione derivabile]\footnote{Slide 10 PDF 15.}
	...
	\begin{equation*}
		...
		\end{aligned}
	\end{equation*}
	Se $f$ è derivabile in $A$ allora
	\begin{equation*}
		\exists\frac{\partial f}{\partial x_i}(\uline x)\qquad \forall\uline x\in A,\, i=1,\hdots, n,
	\end{equation*}
	dove $\frac{\partial f}{\partial x_i}(\uline x)$ è (se esiste finito)
	\begin{equation*}
		\frac{\partial f}{\partial x_i}(\uline x)=\lim_{h\rightarrow 0}\frac{f(x_1,\hdots,x_i+h,\hdots,x_n)-f(\uline x)}{h}.
	\end{equation*}
\end{definition}
