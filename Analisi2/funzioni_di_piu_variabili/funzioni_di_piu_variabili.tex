...
\begin{example}\footnote{Slide 13 PDF 15.}
	...
	\paragraph{Svolgimento:} La derivata direzionale e' il seguente limite (se esiste finito)
	\begin{equation*}
...
\begin{theorem}[Formula del gradiente]
	Sia $f:A\rightarrow\mathbb{R}$, dove $A$ e' un aperto ed $f$ e' differenziabile in $\uline x_0\in A$, allora per ogni versore $\vec{v}$ vale
	\begin{equation*}
...
\begin{example}\footnote{Slide 15 PDF 15.}
	...
	\paragraph{Svolgimento:} Sia $\vec{v}=(v_1,v_2)\in\mathbb{R}^2$ una direzione, ovvero $||\vec{v}||=\sqrt{v_1^2+v_2^2}=1$. Per definizione la derivata direzionale e' il limite, se esiste ed e' finito,
	\begin{equation*}
		\begin{matrix}
...
\begin{definition}[Differenziabilita']\label{def:differenziabilita_funzione_piu_variabili}\footnote{Slide 2 PDF 16.}
	Sia $f:A\subseteq\mathbb{R}^n\rightarrow\mathbb{R}$, dove $A$ e' un aperto e sia $\uline{x} = (x_1,\hdots, x_n)\in A$. $f$ si dice differenziabile in $\uline x$ se
	\begin{itemize}
		\item $f$ e' derivabile in $\uline x\in A$ (ovvero $\exists \triangledown f(\uline x)=\left(\frac{\partial f}{\partial x_1}(\uline x), \hdots, \frac{\partial f}{\partial x_n}(\uline x) \right)$)
		\item
		\begin{equation*}
			\lim_{\uline h\rightarrow \uline 0}\frac{f(\uline x + \uline h) - f(\uline x) - <\triangledown f(\uline x), \uline h>}{||\uline h||} = 0,\quad \uline h\in\mathbb{R}^n.
		\end{equation*}
	\end{itemize}
\end{definition}
...
\begin{definition}[Differeziale]\footnote{Slide 2 PDF 16.}
	Fissato $x\in A$, punto nel quale $f:A\rightarrow\mathbb{R}$ e' differenziabile, l'applicazione (funzionale) lineare$_{\footnotemark}$
	\begin{equation*}
		\begin{aligned}
...
E' riportata un'ulteriore forma di definizione di funzione differenziabile perche' in alcuni libri e' scritta cosi':
\begin{definition}[Funzione differenziabile (alternativa)]
	Sia $f:A\rightarrow\mathbb{R}$, dove $A$ e' aperto, $f$ si dice differeziabile in $\uline x\in A$ se   
\end{definition}
