...
\begin{theorem}[Formula del gradiente]
	Sia $f:A\rightarrow\mathbb{R}$, dove $A$ è un aperto ed $f$ è differenziabile in $\uline x_0\in A$, allora per ogni versore $\vec{v}$ vale
	\begin{equation*}
...
\begin{definition}[Funzione differenziabile (alternativa)]\label{def:funzione_differenziabile_alternativa} \footnote{Slide 2 PDF 16.}
	Sia $f:A\subseteq\mathbb{R}^n\rightarrow\mathbb{R}$, dove $A$ è aperto, $f$ si dice differeziabile in $\uline x\in A$ se esiste un funzionale lineare
	\begin{equation*}
		\begin{aligned}
			L:\mathbb{R}^n&\rightarrow\mathbb{R}\\
			\uline h&\mapsto L(\uline h) = <\triangledown f(\uline x), \uline h>
...
\begin{proof}
	...
	\end{equation}
	\footnotetext{Dato che $f$ è differenziabile in $\uline x_0$ allora è possibile scrivere
	\begin{equation*}
...
\begin{theorem}[del differenziale]\label{th:del_differenziale}
