...
\begin{proof}
	...
	\begin{enumerate}
		...
		\noindent Siano $y_1(x)$ e $y_2(x)$, queste sono linearmente indipendenti perché
		\begin{equation*}
			\begin{matrix}
				\begin{vmatrix}
					y_1(x) & y_2(x)\\
		...
		(Come prima, per dimostrare che $e^{\lambda_1 x} \text{ e } e^{\lambda_2 x}$ generano lo spazio delle soluzioni è necessario mostrare che sono linearmente indipendenti tramte il determinante della matrice Wronskiana.)
		\begin{equation*}
			\begin{vmatrix}
				e^{\lambda_1x} & e^{\lambda_2 x}\\
				\lambda_1 e^{\lambda_1x} & \lambda_2 e^{\lambda_2 x}
...
\paragraph{Riassunto:} Il problema è trovare la solzione di
\begin{equation*}
	ay'' + b y' + cy = 0,
\end{equation*}
...
\footnotetext{Tale condizione imposta sara' una delle condizioni del sistema. Perché è imposta? E' un trucco matematico per facilitare il calcolo di $c_1$ e $c_2$ come si vede da (\ref{eq:punto_inizio_trucco}) in poi.}
