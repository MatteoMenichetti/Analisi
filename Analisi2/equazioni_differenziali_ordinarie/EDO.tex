...
\begin{proof}[Dimostrazione (per $n=1$, \textbf{valido anche per} $\boldsymbol{n>1)}$]\footnote{PDF 2, Slide 1.}
     Sia la EDO lineare completa di ordine $n=1$ in forma normale ((\ref{eq:EDO_lineare_ordine_n_forma_normale}) equivale a)
...
\paragraph{Osservazione sulla forma di (\ref{eq:edo_lineare_secondo_ordine_omogenea_coefficienti_costanti}):}
...
\begin{proposition}\footnote{Slide 3 PDF 5.}\label{prop:e^lambdax_soluzione_omogenea_secondo_grado_lineare}
	\begin{equation*}
		z(x)=e^{\lambda x}\text{ è soluzione (reale o complessa\footnotemark) di (\ref{eq:edo_lineare_secondo_ordine_omogenea_coefficienti_costanti})} \iff \lambda \text{ è radice (zero) di } p(\lambda).
	\end{equation*}
...
\begin{enumerate}
	...
	\begin{equation}\label{eq:integrale_generale_edo_lineare_secondo_ordine_omogenea_coefficienti_costanti_Delta_>_0}
		\boldsymbol{z(x)} = c_1 (x)y_1(x)+ c_2y_2(x) = \boldsymbol{c_1 e^{\lambda_1 x} + c_2 e^{\lambda_2 x}}.
	\end{equation}
	\item \textbf{Caso} $\boldsymbol{\Delta = 0}$:
	...
	\begin{equation}\label{eq:soluzioni_omogenea_secondo_grado_delta_positiva}
		y_1(x) = e^{\alpha x}\cos\beta x\in\mathbb{R},\quad y_2(x) = e^{\alpha x} \sin\beta x\in\mathbb{R},\quad \lambda_{1,2}=\alpha \pm i \beta\in\mathbb{C},
	\end{equation}
	dove
	\begin{itemize}
		...
		\item (Non ufficiale) $y_1$ e $y_2$ sono soluzioni reali dell'omogenea (\ref{eq:edo_lineare_secondo_ordine_omogenea_coefficienti_costanti}). (Mi immagino quanto segue:) Sono state considerate le soluzioni reali per comodità, in quanto equivalenti alle soluzioni immaginarie.
	\end{itemize}
...
\begin{theorem}[Integrale generale EDO lineare omogenea di secondo grado a coeficienti costanti]\footnote{Slide 5 PDF 5.}
	...
	\begin{equation}\label{eq:integrale_generale_edo_lineare_secondo_ordine_omogenea_coefficienti_costanti}
		z(x,c_1,c_2) = c_1y_1(x) + c_2y_2(x),\quad \text{ al variare dei coefficienti } c_1,c_2\in\mathbb{R},
	\end{equation}
...
\begin{proof}
	...
	\begin{enumerate}
		...
		\begin{equation*}
			z(x) = e^{\lambda x}u(x) = e^{\lambda x} (c_1+c_2x) = c_1\cdot \underbrace{e^{\lambda x}}_{(x)} + c_2\cdot\underbrace{x\cdot e^{\lambda x}}_{y_2(x)}.
		\end{equation*}
...
\begin{enumerate}
	...
	\item $\boldsymbol{\Delta = 0}$ allora
	\begin{equation*}
		z(x) = c_1 e^{\lambda x} + c_2xe^{\lambda x},\quad c_1,c_2\in\mathbb{R},\, \lambda_1 = \lambda_2=\lambda=\frac{-b}{2a};
	\end{equation*}
	\item $\boldsymbol{\Delta<0}$ allora
	\begin{equation*}
...
\paragraph{Come fare?} Sia una EDO lineare del secondo ordine a coefficienti costanti del tipo
\begin{equation}\label{eq:edo_metodo_verosomiglianza}
	ay''(x) + by'(x) + c y(x) = f(x),\quad x\in I,
\end{equation}
è necessario distinguere i seguenti casi:
\begin{enumerate}
	\item se $f(x)$ è un polinomio di grado $n$ allora la soluzione particolare sarà del tipo
	\begin{equation*}
		 \bar y(x)=A_n x^{n_1} + A_{n_1-1} x^{n_1-1} + \hdots + A_0,
	\end{equation*}
	dove
	\begin{itemize}
		\item $n_1=n$ se $a,b,c\neq0$,
		\item $n_1 = n+1$ se $c=0$,
		\item $n_1 = n + 2$ se $b=c=0$.
	\end{itemize}
	Per determinare $A_0,\, \hdots,\, A_n$ è necessario derivare due volte $\bar y(x)$, sostituire le derivate in (\ref{eq:edo_metodo_verosomiglianza}) e risolvere il sistema con $n$ equazioni divise in base al grado della $x$ di afferenza. Ad esempio: se sostituendo le derivate nella EDO è ottenuta l'espressione
	\begin{equation*}
		8A + 16(Ax + B) = 3x-2,
	\end{equation*}
	...
	\item se $f(x) = Ae^{\lambda x}$ allora la soluzione particolare sarà del tipo
	\begin{equation*}
		\bar y(x) = c\, e^{\lambda x},
	\end{equation*}
	con $c$ da determinare, dove
	\begin{itemize}
		\item se $\lambda$ è una radice del polinomio caratteristico $p(\lambda)$ allora $\bar y(x)=cx^me^{\lambda x}$, dove $m$ è la molteplicità della radice del polinomio $\lambda$. 
	\end{itemize}
	
	\item se $f(x) = A \cos(\omega x) + B\sin(\omega x)$ allora la soluzione è del tipo
	\begin{equation*}
	...
	\item se $f(x) = e^{\lambda x}\;[A \cos(\omega x) + B\sin(\omega x)]$ allora la soluzione è del tipo
	\begin{equation*}
		\bar y = e^{\lambda x}(c_1 \cos(\omega x) + c_2 \sin(\omega x)),
	\end{equation*}
	dove $c_1$ e $c_2$ sono da determinare.
	
	\item se $f(x)=e^{\lambda x} p(x)$, con $p(x)$ polinomio di grado $n$ allora la soluzione sarà della forma
	\begin{equation*}
		\bar y(x) = e^{\lambda x} q(x),
	\end{equation*}
	con $q(x) = a_nx^n+\hdots + a_0$, dove $a_0,\hdots, a_n$ sono da determinare e se $\lambda$ è radice del polinomio caratteristico allora $q(x)$ sarà di grado $n+1$.
\end{enumerate}
...
\begin{equation*}
	y(x) = z(x) + \bar y(x) =  \underbrace{c_1(x) y_1(x) + c_2(x) y_2(x)}_{(\ref{eq:integrale_generale_edo_lineare_secondo_ordine_omogenea_coefficienti_costanti})} + \underbrace{c_1(x) y_1(x) + c_2(x) y_2(x)}_{(\ref{eq:soluzione_particolare_edo_secondo_ordine_non_omogenea_variazione_costanti})},\quad c_1,c_2\in\mathbb{R}.
\end{equation*}
...
\subsubsection{Esercizi EDO secondo ordine}
\begin{example}\footnote{Slide 17 PDF 6.}
	...
	ovvero
	\begin{equation*}
		24 A e^{2x} = 3 e^{2x}\quad\Rightarrow\quad 24 A  = 3 \quad\Rightarrow\quad A=\frac{3}{24} = \frac{1}{8}.
	\end{equation*}
	...
	\begin{equation*}
		y'(x) = \frac{2}{3} c_1 e^{-\frac{2}{3}x}  c_1 e^{-x} + \frac{1}{4}e^{2x}.
	\end{equation*}
...
\begin{example}[Caso $\Delta<0$]\footnote{Slide 23 PDF 6.}
	...
	\begin{equation*}
		...
		\end{cases}
	\end{equation*}
	\paragraph{Nota:} Se $y(0)$ e $y'(0)$ fossero uguali allora la soluzione sarebbe nulla.
	\paragraph{Svolgimento:} Sia la EDO lineare di secondo ordine a coefficienti costanti
