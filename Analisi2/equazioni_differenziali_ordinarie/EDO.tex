...
\begin{proof}\footnote{PDF 2, Slide 1.}
    ...
    \end{equation*}
    è una soluzione particolare della EDO omogenea (\ref{eq:EDO_lineare_omogenea_associata_ordine_1_forma_normale}). Siano
    \begin{equation*}
...
\begin{enumerate}
	...
	\begin{itemize}
		\item Se la EDO è a variabili separabili:
		\begin{enumerate}
			\item Separare le variabili,
			\item Determinare la forma esplicita della $y$,
		\end{enumerate}
...
\paragraph{Come si separano le variabili di una EDO lineare a variabili separabili della forma (\ref{eq:EDO_primo_ordine_forma_normale_variabili_separabili})?} Per "separazione" delle variabili si intende che la $x$ deve essere separata dalla $y(x)$. La separazione è svolta con i seguenti passaggi:
\begin{enumerate}
    ...
    \begin{equation*}
        G(y(x)) = F(x) + c.
    \end{equation*}
    \item (L'obiettivo è esplicitare $x$, quindi:) Se $G$ è invertibile si ha (la soluzione in forma implicita)
    \begin{equation*}
        y(x) = G^{-1}(x) [F(x) + c].
    \end{equation*}
    \item Esplicitando $y(x)$ è trovata la soluzione del problema.
\end{enumerate}
...
\paragraph{Procedura su come risolvere problemi di Cauchy del primo ordine:}
\begin{enumerate}
	\item Se è a variabili separabili 
\end{enumerate}
