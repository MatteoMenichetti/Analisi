...
\newglossaryentry{R0+}{
    name=$\mathbb R_0^+$,
    description = {$= \mathbb R^+\cup\{0\}$}
}

\newglossaryentry{R}{
    name=$\mathbb R$,
    description ={$= \mathbb Q \cup\{x:x\notin \mathbb Q\}$}
}

\newglossaryentry{Rext}{
    name = $\overline{\mathbb R}$,
    description = {$=\mathbb R\cup\{\pm\infty\}$ è detto $\mathbb R$ esteso}
}
...
\newglossaryentry{q}
{
    name=$\boldsymbol{\mathbb Q}$,
    description={$=\left\{\frac{p}{q}:p\in\mathbb Z,\, q\in\mathbb N \right\}$}
}
...
\newglossaryentry{sottospazio}{
    ...
    description = {Un sottospazio vettoriale è un sottoinsieme di uno spazio vettoriale ereditandone le proprietà}
}
...
\newglossaryentry{spazio lineare}{
	name = spazio lineare,
	description={Sinonimo di Spazio Vettoriale}
}

\newglossaryentry{traccia}{
	name = traccia,
	description={Sia $A\in\mathbb{R}^{(n\times n)}$, allora la traccia è la somma degli elementi sulla diagonale principale, ovvero $tr(A)=\sum _{i=1}^{n}a_{ii}$}
}

\newglossaryentry{soluzioni complesse coniugate}{
	name = soluzioni complesse coniugate,
	description={Dato $p(\lambda)$ polinomio a coefficienti reali (definito come (\ref{eq:equazione_caratteristica_associata_omogenea})) e $\alpha + i \beta$ una sua radice, allora il \gls{complesso coniugato} $\alpha - i\beta$ è anch'esso radice di $p(\lambda)$}
}
...
\include{Analisi1/main}

\include{Analisi2/main}
